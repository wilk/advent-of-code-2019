...
const santaAncestors = [];
let cursor = 'SAN';
let level = 0;
while (cursor !== 'COM') {
  santaAncestors.push({ node: orbitMap[cursor], level });
  cursor = orbitMap[cursor];
  level++;
}
...
while (!commonAncestor) {
  const ancestor = santaAncestors.find(el => el.node === cursor);
  if (ancestor) {
    commonAncestor = true;
    total = ancestor.level + level - 1;
  }
