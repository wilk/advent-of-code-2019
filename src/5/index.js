...
Your puzzle answer was 13294380.
...
Your puzzle answer was 11460760.
*/
...
const JUMP_TRUE_INSTRUCTION = 5;
const JUMP_FALSE_INSTRUCTION = 6;
const LESS_THAN_INSTRUCTION = 7;
const EQUALS_INSTRUCTION = 8;
...
const programs = {
  ...
  [JUMP_TRUE_INSTRUCTION]: (i, program, modes) => {
    const checkValue = modes[0] === 0 ? program[program[i]] : program[i];
    const pointer = modes[1] === 0 ? program[program[i + 1]] : program[i + 1];
    const nextPointer = checkValue !== 0 ? pointer : i + 2;
    return [program, true, nextPointer];
  },
  [JUMP_FALSE_INSTRUCTION]: (i, program, modes) => {
    const checkValue = modes[0] === 0 ? program[program[i]] : program[i];
    const pointer = modes[1] === 0 ? program[program[i + 1]] : program[i + 1];
    const nextPointer = checkValue === 0 ? pointer : i + 2;
    return [program, true, nextPointer];
  },
  [LESS_THAN_INSTRUCTION]: (i, program, modes) => {
    const firstValue = modes[0] === 0 ? program[program[i]] : program[i];
    const secondValue = modes[1] === 0 ? program[program[i + 1]] : program[i + 1];
    const output = firstValue < secondValue ? 1 : 0;
    program[program[i + 2]] = output;
    return [program, true, i + 3];
  },
  [EQUALS_INSTRUCTION]: (i, program, modes) => {
    const firstValue = modes[0] === 0 ? program[program[i]] : program[i];
    const secondValue = modes[1] === 0 ? program[program[i + 1]] : program[i + 1];
    const output = firstValue === secondValue ? 1 : 0;
    program[program[i + 2]] = output;
